...
/*   Updated: 2026/01/14 12:01:41 by pberne           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (token_lst)
	{
		...
		if (token->type == token_op)
		{
			validity_id = ft_is_operator_valid((t_token_op_type *)token->data,
					(t_parsing_token *)token_lst->next->content, prev);
			if (!validity_id)
				return (0);
			if (validity_id == 2)
				ft_merge_redirection_token(token_lst);
		}
		prev = (t_parsing_token *)token_lst->content;
		token_lst = token_lst->next;
	}
	return (1);
}
...
t_list	*ft_token_validity_checker(t_list *token_lst)
{
	...
	if (!ft_verify_subshell_pipelines(token_lst))
	{
		ft_free_token_list(token_lst);
		return (0);
	}
	ft_remove_end_token(&token_lst);
