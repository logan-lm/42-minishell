...
/*   Updated: 2026/01/13 18:49:15 by pberne           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_is_cmpd_removeable(t_string_compound_lst **cmpd)
{
	...
	if ((ft_is_cmpd_always_naked(*cmpd) || ft_is_always_expanded(*cmpd)) && ft_is_cmpd_always_empty(*cmpd))
		return (1);
	else
	{
		if (ft_is_cmpd_always_empty(*cmpd))
		{
			...
			temp->is_expanded = ft_is_always_expanded(*cmpd);
			*cmpd = temp;
		}
		else
